...
//@note - this rule always fails because first expr is always true
invariant twoBalancesGreaterThanSingle(address account1, address account2)
    ...
    balanceOf(account1) + balanceOf(account2) <= balanceOf(account1)
...
rule transferOutDoesNotChangePowerBalance(address user1, address user2, address user3, uint256 amount){
    env e;
    calldataarg args;
    uint256 _balance1 = balanceOf(user1);
    uint256 _balance2 = balanceOf(user2);
    require e.msg.sender == user1;

    //@note - there is an error in constrain of balance2 which makes _balance1 == _balance2 be true
    //        but then the require statement cuts off all pathways so the rule always pass and never reaches asserts
...
rule lastRevertedExample(address sender, address recipiecnt, uint256 amount){
    ...
    //@note - one needs to cache lastReverted value
    transferFromReverted = lastReverted

    uint256 allownce_ = allowance(sender, recipiecnt);

    assert transferFromReverted => _allownce < amount;
}
...
rule balanceOfChange(method f, address user1, address user2) {
    ...
    //@note - this can be rewritten to
    // require !((!(balanceOf1Before < balanceOf2Before)) && (!(balanceOf2Before < balanceOf1Before)));
    require balanceOf1Before < balanceOf2Before || balanceOf2Before < balanceOf1Before;
    //@note which cuts off all (among others) pathways where user1 == user2
    ...
    //@note - user1 != user2 always true
    assert ((balanceOf1After != balanceOf1Before) && (balanceOf2After != balanceOf1Before)) => user1 != user2;
}
